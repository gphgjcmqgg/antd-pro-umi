...
class Message {
  public success(content: ConfigContent, onClose?: ConfigOnClose) {
    message.open({ content: <ABSParagraph style={{display: 'inline'}}>{content}</ABSParagraph>, duration: durationNumber, type: 'success', onClose, icon: <IconNode type="check-circle" />});
  }
  public error(content: ConfigContent, onClose?: ConfigOnClose) {
    message.open({ content: <ABSParagraph style={{display: 'inline'}}>{content}</ABSParagraph>, duration: durationNumber, type: 'error', onClose, icon: <IconNode type="close-circle" />});
  }
  public warning(content: ConfigContent, onClose?: ConfigOnClose) {
    message.open({ content: <ABSParagraph style={{display: 'inline'}}>{content}</ABSParagraph>, duration: durationNumber, type: 'warning', onClose, icon: <IconNode type="exclamation-circle" />});
  }
  public loading(content: ConfigContent, onClose?: ConfigOnClose) {
