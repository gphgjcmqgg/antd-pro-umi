...
class ABSPagination extends React.Component<IABSPaginationProps, IABSPaginationState> {
  static defaultProps = {
    pageSize: PAGE_SIZE,
  };
...
const mapStateToProps = (state, { model, contentData, pageSize }) => {
  if (model && model.length > 0) {
    ...
    if (data && Array.isArray(data)) {
      return { total: data.length, current: 1, pageSize, loading: state.loading.models[modelName] };
    } else {
      // 带分页的列表
      const { current_page, total_items, page_size } = data;
      return { total: total_items, current: current_page, pageSize: page_size, loading: state.loading.models[modelName] };
    }
  }
...
export default connect(mapStateToProps, null, null, { withRef: true })(ABSPagination);