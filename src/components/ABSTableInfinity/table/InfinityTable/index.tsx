...
export interface IInfinityTableProps {
  // loading 效果
  loadingIndicator?: any;
  onScroll?: (e: any) => void; // 滚动事件
  onFetch?: () => void; // 滚动到低部触发Fetch方法
  sumData?: any; // 合计行
  dataSource?: any;
  columns?: any;
  forwardedRef?: any;
...
export interface IInfinityTableState {
  // recordIndex: number;
}
...
export class InfinityTable extends PureComponent<IInfinityTableProps, IInfinityTableState> {
  static defaultProps = {
  ...
  refScroll;
  refTable;
  setStateWithThrottle;
  ioTargetState;
  refUnderPlaceholder = document.createElement('div');
  refUpperPlaceholder = document.createElement('div');
  io;
  ...
  static PlaceHolder({ height, domNode, loading, loadingIndicator }: any) {
    return (
      domNode &&
  ...
  componentDidMount() {
    /* eslint-disable */
    this.refScroll = (ReactDOM.findDOMNode(this.refs.current) as any).getElementsByClassName(
      'ant-table-body',
    )[0];
    ...
    if (this.props.onScroll && this.refScroll.addEventListener('scroll', this.props.onScroll)) {
      if (this.refScroll) {
  ...
  componentWillUnmount() {
    if (this.props.onScroll && this.refScroll.removeEventListener('scroll', this.props.onScroll)) {
  ...
  toggleObserver(condition: boolean = true) {
    if (condition) {
    ...
    return (
      <Fragment>
        <InfinityTable.PlaceHolder
          height={Number.isNaN(upperHeight) ? 0 : upperHeight}
          domNode={this.refUpperPlaceholder}
        />
        <InfinityTable.PlaceHolder
          height={Number.isNaN(underHeight) ? 0 : underHeight}
