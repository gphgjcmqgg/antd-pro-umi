...
export class ABSStructure extends React.Component<IStructureProps, IStructureState> {
  ...
  render() {
    ...
    const clazz = classNames('abs-structure', {'abs-structure-border': this.props.hasBorder });
    return (
      <div className={clazz}>
  ...
  buildStructureHtml(isShot: boolean) {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      layerData = this.getLayerWithSecurityType(that.state.structureState.data);
  ...
  // 按照证券级别分（优先夹层等）
  getLayerWithSecurityType(data: any) {
    let allData: INoteData[] = [];
    let typeArr: any[] = [];
    data.forEach(element => {
      if (typeArr.indexOf(element.securityTypeId) < 0) {
        typeArr.push(element.securityTypeId);
      }
    });
    typeArr.forEach(item => {
      var ratingArr = this.getRatingArrBySecurityType(item, data);
      ratingArr.forEach(r => {
        allData.push(data.filter(d => {
          return d.securityTypeId === item && d.rating === r;
        }));
      });
    });
    return allData;
  ...
  // 按照等级获取评级Array
  getRatingArrBySecurityType(type: any, data: any) {
    var ratingArr: any[] = [];
    data.forEach(element => {
      if (element.securityTypeId === type && ratingArr.indexOf(element.rating) < 0) {
